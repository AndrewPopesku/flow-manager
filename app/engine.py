...
class FlowEngine:
    ...
    def run(self):
        ...
        while current_task_name and current_task_name != "end":
            ...
            try:
                ...
                print(f"Error executing task '{current_task_name}': {e}")
    ...
    def _evaluate_next_task(self, current_task_name: str, result: TaskResult) -> str:
        ...
        """
        # Find condition for the current task
        # Assuming one condition per task for simplicity based on the prompt's structure,
        # but the model allows a list. We need to find the condition where source_task matches.
        ...
        if not relevant_condition:
            # If no condition is defined, we can't proceed automatically unless we assume linear flow,
            # but the prompt implies conditions drive the flow.
            # If no condition, maybe we just stop or look for the next task in the list?
            # The prompt says: "The flow must include conditions to evaluate the results... and determine whether to proceed"
            # So if no condition, we probably end or it's a terminal task.
            print(f"No condition found for task '{current_task_name}'. Ending flow.")
            return "end"
