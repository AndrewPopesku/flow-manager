...
class Task(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Description of what the task does"
    )
...
class Condition(BaseModel):
    ...
    source_task: str = Field(
        ..., description="The name of the task whose result is evaluated"
    )
    outcome: str = Field(
        ..., description="The expected outcome of the source task (e.g., 'success')"
    )
    target_task_success: str = Field(
        ..., description="The next task to execute if the outcome matches"
    )
    target_task_failure: str = Field(
        "end", description="The next task to execute if the outcome does not match"
    )
...
class FlowBase(BaseModel):
    ...
    conditions: List[Condition] = Field(
        ..., description="List of conditions governing flow logic"
    )
